...
+ (id)modelWithModelNamed:(CPString) aModelName bundle:(CPBundle)aBundle
{
    ...
    CPLog.debug(modelURL);
    CPLog.debug(aModelName);
	if(modelURL != nil)
	{
...
{
    //Unimplemented
    CPLog("mergedModelFromBundles called on CPManagedObjectModel")
	return nil
}
...
{
	//Unimplemented
    CPLog("modelByMergingModels called on CPManagedObjectModel")
    return nil;
}
