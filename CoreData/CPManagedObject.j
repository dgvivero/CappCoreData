...
@implementation CPManagedObject : CPObject
{
	...
	CPString _id @accessors(property=couchID);
	CPString _rev @accessors(property=couchREV);
...
- (void)takeStoredValue:(id)value forKey:(CPString)aKey
{
	...
	{		
		var values;
		if([value isKindOfClass:[CPMutableSet class]])
		{
			values = [value allObjects];
		}
		else if([value isKindOfClass:[CPMutableArray class]])
		{
			values = value;
		}
