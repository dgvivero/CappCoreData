...
{
	var aJSO= [self serializeToDictionary:containsAllProperties containsChangedProperties:containsChangedProperties];

    var result =  [CPString JSONFromObject:aJSO];
	return result;
}
...
+ (id)deserializeFromDictionary:(CPDictionary) aDictionary withContext:(CPManagedObjectContext) aContext
{
	...
	if(aDictionary != nil)
	{
		...
		{		
			...
			var objCouchID = [aDictionary objectForKey:@"_id"];
			var objCouchREV = [aDictionary objectForKey:@"_rev"];
			...
			[aCPManagedObject setCouchID:objCouchID];
			[aCPManagedObject setCouchREV:objCouchREV];
...
- (CPDictionary)serializeToDictionary:(BOOL) containsAllProperties containsChangedProperties:(BOOL)containsChangedProperties
{
    ...
    if ([self couchID] != null) [result setObject:[self couchID] forKey:"_id"];
    if ([self couchREV] != null) [result setObject:[self couchREV] forKey:"_rev"];
...
- (CPDictionary) serializeProperties:(CPDictionary) aDictionary
{
	...
	{
		var aObject = [aDictionary objectForKey:aKey];
		if(aObject != null && aObject != nil && aObject != CPNull && aObject)
		{
